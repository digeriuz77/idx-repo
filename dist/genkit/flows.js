...
exports.sessionFeedbackFlow = exports.miCoachingFlow = exports.streamingPersonaChatFlow = exports.generatePersonaFlow = void 0;
 ...
 */
exports.generatePersonaFlow = ai.defineFlow({
    name: "generatePersonaFlow",
    inputSchema: schemas_1.PersonaInputSchema,
    outputSchema: schemas_1.PersonaOutputSchema,
}, async (input) => {
    const result = await ai.generate({
        ...
        output: {
            schema: schemas_1.PersonaOutputSchema,
            format: 'json'
        },
    });
 ...
 */
exports.streamingPersonaChatFlow = ai.defineFlow({
    name: "streamingPersonaChatFlow",
    inputSchema: schemas_1.ChatInputSchema,
    // Type as any to workaround the type error for now
    streamSchema: genkit_1.z.object({
    ...
    ${input.conversation_history ? `Previous conversation:\n${input.conversation_history.map(msg => `${msg.role === 'user' ? 'User' : 'You'}: ${msg.content}`).join('\n')}` : ''}
 ...
 */
exports.miCoachingFlow = ai.defineFlow({
    name: "miCoachingFlow",
    inputSchema: schemas_1.CoachingInputSchema,
    outputSchema: schemas_1.CoachingOutputSchema,
}, async (input) => {
    const result = await ai.generate({
        model: googleai_1.gemini20Flash,
      ...
      If has_coaching is false, the coaching_message, mi_technique_used, and missed_opportunity fields can be empty strings.`,
        output: {
            schema: schemas_1.CoachingOutputSchema,
            format: 'json'
        },
    });
    return result.output || {
 ...
 */
exports.sessionFeedbackFlow = ai.defineFlow({
    name: "sessionFeedbackFlow",
    inputSchema: schemas_1.SessionFeedbackInputSchema,
    outputSchema: schemas_1.MITIScoreSchema,
}, async (input) => {
    const result = await ai.generate({
        model: googleai_1.gemini20Flash, // Use the same model as other flows
      ...
      Highlight 2-3 specific strengths and 2-3 areas for improvement.`,
        output: {
            schema: schemas_1.MITIScoreSchema,
            format: 'json'
        },
    });
    return result.output || {
